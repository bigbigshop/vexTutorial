...
**修正說明：**
1. **標籤閉合**：我確保了每個 `<TabItem>` 的結尾都有正確的換行，避免與內部的 ` ``` ` 代碼塊黏在一起。
2. **語法校對**：修正了 PROS 4 代碼中 `double` 運算的精度問題（例如將 `127` 改為 `127.0` 以防整數除法誤差）。
3. **格式連貫**：補回了被截斷的 Markdown 標記（如加粗、列表）。

這樣複製過去 Docusaurus 應該就能正常渲染了。下一步你想處理 **5.3 PID 演算法** 還是 **5.4 感應器應用**？